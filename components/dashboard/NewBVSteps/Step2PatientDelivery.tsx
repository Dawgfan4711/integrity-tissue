...
export function Step2PatientDelivery({ onNext, onBack, onClose, defaultValues }: Step2PatientDeliveryProps) {
  ...
  return (
    <div className="p-8">
      <div className="flex justify-between mb-4">
        <button className="text-sm text-gray-600" onClick={onBack}>&larr; Back</button>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
          <FormField name="initials" control={form.control} render={({ field }) => (
            <FormItem>
          ...
          <FormField name="applicationDate" control={form.control} render={({ field }) => (
            <FormItem>
          ...
          <div className="flex gap-4 mt-6">
            ...
            <Button type="submit" className="flex-1 bg-blue-600 hover:bg-blue-700 text-white">Get Product Recommendation</Button>
          </div>
        </form>
      </Form>
    </div>
